...
enum DIRECTIONS {EAST, NORTHEAST, NORTH, NORTHWEST, WEST, SOUTHWEST,
                   SOUTH, SOUTHEAST }
void main()
{
    int now, ult = 0;
...
int multiply(int i, int j, int direction, int countdown)
{
    if (stuff[i][j] == -1) return 0;
    if (countdown == 0) return 1;
    return stuff[i][j] * multiply(i+directiony[direction],
                                  j+directionx[direction],
                                  direction, countdown-1);
}
